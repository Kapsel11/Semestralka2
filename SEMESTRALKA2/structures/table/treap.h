...
namespace structures
{
	...
	inline void Treap<K, T>::insert(const K & key, const T & data)
	{
		...
		BSTTreeNode* binTreeNode = new BSTTreeNode(treapItem);

		if (tryToInsertNode(binTreeNode))
		{
			size_++;
			while (!isHeapOK(binTreeNode))
			{
				if (binTreeNode->isLeftSon())
				{
					rotateRightOverParent(binTreeNode);
				}
				else
				{
					rotateLeftOverParent(binTreeNode);
				}
			}
		}
		else
