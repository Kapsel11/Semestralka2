...
namespace structures
{
	...
	inline BinarySearchTree<K, T>& BinarySearchTree<K, T>::operator=(const BinarySearchTree<K, T>& other)
	{
		if (this != &other)
		{
			...
			Tree<TableItem<K, T>*>::PreOrderTreeIterator iterCurrent(other.binaryTree_->getRoot());
	...
	inline T & BinarySearchTree<K, T>::operator[](const K key)
	{
		bool found = false;
		BSTTreeNode* data;
		data = findBSTNode(key, found);
		if (found)
		{
			return data->accessData()->accessData();
		}
		else
		{
	...
	inline void BinarySearchTree<K, T>::insert(const K & key, const T & data)
	{
		...
		BSTTreeNode* binTreeNode = new BSTTreeNode(tableItem);

		if (tryToInsertNode(binTreeNode))
		{
			size_++;
		}
		else
		{
			delete tableItem;
			delete binTreeNode;
			throw std::logic_error("Key already there!");
		}
	}
