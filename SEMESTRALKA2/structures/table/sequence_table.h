...
namespace structures 
{
	...
	inline void SequenceTable<K, T>::insert(const K & key, const T & data)
	{
		if (containsKey(key))
		{
			throw std::logic_error("Table already contains data with the same key!");
		}
		else
		{
	...
	inline T SequenceTable<K, T>::remove(const K & key)
	{
		...
		if (item == nullptr)
		{
			std::logic_error("Cant remove item with specified key because it doesnt exist!");
		}
		else
		{
	...
	{
		TableItem<K, T>* p = findTableItem(key);
		if (p != nullptr)
		{
			data = p->accessData();
			return true;
