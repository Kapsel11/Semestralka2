...
namespace structures
{
	...
	{
		//TODO 04: LinkedList
		LinkedListItem<T>* mazany = nullptr;
		if (index == 0 && size_ > 0)
		{
			mazany = first_;
			...
			if (mazany == last_) { //if (mazany == nullptr) {
				last_ = pred;
			}
		}
	...
	void LinkedList<T>::set(const int index, const T& data) {
		...
		LinkedListItem<T>* pred = getItemAtIndex(index - 1);
		LinkedListItem<T>* novy = new LinkedListItem<T>(data);
		novy->setNext(pred->getNext()->getNext());
		delete pred->getNext();
		pred->setNext(novy);
	}
}