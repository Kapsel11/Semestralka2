...
namespace structures {
	...
	{
		DSRoutines::rangeCheckExcept(count + index, size_ + 1, "Invalid index!");
	...
	void Vector::copy(const Vector& src, const int srcStartIndex, Vector& dest, const int destStartIndex, const int length)
	{
		...
		{
			memmove(dest.getBytePointer(destStartIndex),
				src.getBytePointer(srcStartIndex), length);
		}
		else
		{
			memcpy(dest.getBytePointer(destStartIndex),
				src.getBytePointer(srcStartIndex), length);
		}
	}
