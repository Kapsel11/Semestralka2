...
namespace structures
{
	...
	{
		return parent_ != nullptr && getParent()->getLeftSon() == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::isRightSon() const
	{
		return parent_ != nullptr && getParent()->getRightSon() == this;
	}
	...
	inline void BinaryTree<T>::InOrderTreeIterator::populatePath(TreeNode<T>* const current)
	{
		if (current != nullptr)
		{
			BinaryTreeNode<T>* currentBinaryTreeNode = dynamic_cast<BinaryTreeNode<T>*>(current);

			populatePath(currentBinaryTreeNode->getLeftSon());

			path_->push(current);

			populatePath(currentBinaryTreeNode->getRightSon());
		}
	}
